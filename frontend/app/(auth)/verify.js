...
export default function VerifyScreen() {
  ...
  const handleVerify = async () => {
    ...
    if (fullCode.length !== 6) {
      showModal({ type: 'warning', title: 'Error', message: 'Please enter the complete 6-digit code' });
      return;
    }
    ...
    if (codeExpired) {
      showModal({
        type: 'warning',
        title: 'Code Expired',
        message: 'This verification code has expired (valid for 24 hours only).\n\nPlease contact your landlord to request a new verification code.'
      });
      return;
    }
    ...
    if (result.success) {
      showModal({
        type: 'success',
        title: 'Success',
        message: 'Email verified! Welcome to Wattipid.',
        onPrimaryPress: () => router.replace('/(tenant)/dashboard')
      });
    } else {
      ...
      if (result.status === 'expired') {
        ...
        showModal({ type: 'error', title: 'Verification Failed', message: result.message });
      }
    }
  };
  ...
  const handleResendCode = async () => {
    ...
    if (result.success) {
      ...
      showModal({ type: 'error', title: 'Error', message: result.message || 'Failed to resend code.' });
    }
  };
      ...
      <View style={s.content}>
        ...
        <View style={s.infoBox}>
          ...
          <Text style={s.infoText}>
            For security, verification codes are valid for 24 hours only. New codes must be manually requested from your landlord.
          </Text>
        </View>
      </View>
