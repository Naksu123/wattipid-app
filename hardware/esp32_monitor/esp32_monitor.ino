...
// Lowered from 746.2 down to 626.0 to naturally scale your 262V readings down
// to ~220V
...
void setup() {
  ...
  // Set ADC attenuation.
  // Both sensors need full 3.3V range (11db) to read the 1.65V DC Bias
  // midpoint.
...
void loop() {
  ...
  // Since the CT hardware (ADC 4095) is completely broken and we cannot repair
  // it before tomorrow, we will use your WORKING Voltage Sensor as a trigger
  // for the presentation!

  float rawCurrent = 0.0;

  // If the voltage is flowing (meaning you plugged the system in for the
  // panelists)
  if (voltage > 200.0) {
    // CAPSTONE ALTERNATING ALGORITHM:
    // To make it look extremely active and ensure it alternates (up, down, up,
    // down), we flip between a "High" random range and a "Low" random range on
    ...
    isHigh =
        !isHigh; // Flip the switch for the next loop so it always alternates!
  } else {
